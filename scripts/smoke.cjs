...
async function run(baseline,mobile){
 ...
 const mods=new Map();function get(file){if(mods.has(file))return mods.get(file);let code=fs.readFileSync(root+'/'+file,'utf8');if(baseline&&file==='app.js')code=fs.readFileSync(process.env.BASELINE_APP,'utf8');if(baseline&&file==='app.js')code+='\nexport { saveTrackBlobToIphone, savePlaylistToIphone, resumeMobileDownloads, retryMobileDownloadFailures, cancelMobileDownload };'; if(!baseline&&file==='mobile-downloads.js')code+='\nexport { retryMobileDownloadFailures, cancelMobileDownload };';const m=new vm.SourceTextModule(code,{context:ctx,identifier:file});mods.set(file,m);return m;}
 const entry=get('app.js');await entry.link((s)=>get(s.replace('./','')));await entry.evaluate();
 const storage=mods.get('storage.js').namespace;const {state,storageKeys}=mods.get('state.js').namespace;
 ...
 const mobileApi=mods.get(baseline?'app.js':'mobile-downloads.js').namespace;
